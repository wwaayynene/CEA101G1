	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				String url = "/back-end/item_type/listOneItem_type.jsp";
...
//				}
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/item_type/addItem_type.jsp");
					failureView.forward(req, res);
					return;
				}
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("item_typeVO", item_typeVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/item_type/update_item_type_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			try {
				...
				/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
				req.setAttribute("item_typeVO", item_typeVO); // 資料庫取出的empVO物件,存入req
				String url = "/back-end/item_type/update_item_type_input.jsp";
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/item_type/listAllItem_type.jsp");
				failureView.forward(req, res);
			}
		}
	}
