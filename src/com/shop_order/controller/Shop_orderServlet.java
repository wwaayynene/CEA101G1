	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/shop_order/select_shop_order.jsp");
				...
				String url = "/back-end/shop_order/listOneShop_order.jsp";
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			try {
				...
				/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
				req.setAttribute("shop_orderVO", shop_orderVO); // 資料庫取出的empVO物件,存入req
				String url = "/back-end/shop_order/update_shop_order_input.jsp";
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/shop_order/listAllShop_order.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("getSp_odnoByMb_id_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			try {
				...
				String url = "/back-end/shop_order/listSp_odnoByMb_id.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); // 成功轉交 listOneEmp.jsp
				successView.forward(req, res);
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/shop_order/update_shop_order_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
				...
				String url = "/back-end/shop_order/listAllShop_order.jsp";
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("shop_orderVO", shop_orderVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/shop_order/addShop_order.jsp");
					failureView.forward(req, res);
					return;
				}
